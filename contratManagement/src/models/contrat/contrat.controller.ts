...
export class ContratController {
    constructor(
        ...
        private externaleService: ExternalApiService
    ) {}

    @Post()
    async create(@Body() createContratDto: CreateContratDto) {
        let client = await this.externaleService.checkClient(
            createContratDto.clientId
        );

        if (client.data) {
            const contrat = await this.contratService.create(createContratDto);
            const updateClient = {
                noSiret: createContratDto.clientId,
                idContrat: contrat._id
            };
            const updatedClient = await this.externaleService.updateClient(
                updateClient
            );
            return contrat;
        } else {
