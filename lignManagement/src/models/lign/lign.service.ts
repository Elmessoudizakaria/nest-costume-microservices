...
export class LignService {
    constructor(
        @InjectModel('Lign') private readonly lignModel: Model<Lign>,
        private externalService: ExternalService
    ) {}

    async create(createLignDto: CreateLignDto) {
        const contrat = await this.externalService.checkoutContrat(
            createLignDto.contratId
        );
        if (contrat.data) {
    ...
    async updateStatus(id: string, status: string) {
        ...
        switch (lign.status) {
            case 'activated':
                updatedContrat.activatedLigns = updatedContrat.activatedLigns.filter(
                    e => e !== id
                );
                break;
            case 'disabled':
                updatedContrat.disabedLigns = updatedContrat.disabedLigns.filter(
                    e => e !== id
                );
                break;
            case 'waiting':
                updatedContrat.waitingLigns = updatedContrat.waitingLigns.filter(
                    e => e !== id
                );
                break;
            default:
        ...
        return await this.lignModel.findOneAndUpdate(
            { _id: id },
            { status: status }
        );
    }
